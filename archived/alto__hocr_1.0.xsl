...
<xsl:function name="mf:getBox">
    ...
    <xsl:variable name="left" select="if (number($HPOS) = number($HPOS)) then number($HPOS) else 0"/>
    <xsl:variable name="top" select="if (number($VPOS) = number($VPOS)) then number($VPOS) else 0"/>
    <xsl:variable name="width" select="if (number($WIDTH) = number($WIDTH)) then number($WIDTH) else 0"/>
    <xsl:variable name="height" select="if (number($HEIGHT) = number($HEIGHT)) then number($HEIGHT) else 0"/>
    ...
    <xsl:variable name="wconfString" select="if ($WC != '' and $wconf = $wconf) then concat('; x_wconf ', string($wconf * 100)) else ''"/>
